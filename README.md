...
- 三个类别(内部代号 A/B/C)现在各接了一批真实的静观练习音频:A=静观饮食(7 条)、B=身体扫描(9 条)、C=静观呼吸(11 条)。点击类别按钮会从对应的一批里随机抽一条播放。原始文件名(通常包含来源、时长、机构等信息)保留在 `js/app.js` 每条记录的 `label` 字段里;实际部署用的文件名统一改成了 `A1.mp3`、`A2.mp3`...这种形式,避免中文/空格/特殊符号在网址里可能出问题。后续往对应的 `audio/A/`、`audio/B/`、`audio/C/` 文件夹里加文件、再在配置里加一行即可继续扩充。
